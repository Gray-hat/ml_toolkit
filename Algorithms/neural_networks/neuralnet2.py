...
class NeuralNetwork():
	...
	def test(self):
		...
		for data in self.test_set[0]:
			...
			if round(m_output,0) == d_output == class_p:
				tp += 1
			elif round(m_output,0) == d_output == class_n:
				tn += 1
			elif round(m_output,0) == class_p and d_output == class_n:
				fp += 1
			elif round(m_output,0) == class_n and d_output == class_p:
				fn += 1	
			else:
...
if __name__ == '__main__':
	...
	training_set_location = os.path.join(location, 'extras/files/dia.csv')
...
ax.set_title('Line graph to show average error')
